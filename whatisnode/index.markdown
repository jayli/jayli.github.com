...
> 或许有些人对上文提到的问题不以为然，因为你知道有很多方法来规避直接eval()带来的问题，你会使用诸如[JSON.parse()](http://stackoverflow.com/questions/1843343/json-parse-vs-eval)来代替eval()。同样有很多[令人信服的论据](http://javascriptweblog.wordpress.com/2010/04/19/how-evil-is-eval/)鼓励我们小心的使用eval()。这些东东都是值得进一步讨论的。但不管怎样，看一看eval()带来了太多类似[栈溢出（Stack Overflow）](http://stackoverflow.com/)这类的问题吧，你会发现大部分程序员并未正确或者安全的使用eval()。这着实是一个问题。因为太多菜鸟程序员似乎根本没有意识到eval()的问题有多严重。
...
此外，Node也计划实现“进程分支”（process forking），[HTML5 Web Workers API](http://www.whatwg.org/specs/web-apps/current-work/complete/workers.html)为更复杂的进程控制提供了引擎（规范）支持。同样，如果你采用基于事件的模型来架构web应用，你的程序可能至少有100多个场景需要线程的支持。最终你会发现，你的编程思路和思考问题的方式发生了改变，你的注意力将放在服务器端处理请求的逻辑上，而不必在乎Node如何工作。
...
- [Node:Up and Running](http://radar.oreilly.com/2010/11/new-directions-in-web-architec.html)
...
- [JavaScript spread to the edges and became permanent in the process](http://radar.oreilly.com/2011/06/javascript-edges-permanent-james-duncan.html)
- [What is Node.js and what does it do?](http://www.youtube.com/watch?v=OUCHr2H-7_g)
