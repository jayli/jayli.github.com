...
CSS3引入了弹性盒模型的概念，这是一种显示内容的全新的方式。因为它将一些在GUI框架（如Adobe Flex）中的已经应用了一段时间的新特性引入到CSS中了，所以这个确实有用。传统上，如果要使一个列表水平排列，那么就要用浮动。弹性盒模型可以做到更多效果和功能。看一下如下代码：
...
创建一个#content的div，它将是应用程序的主要部分。我们希望它在页面中x和y方向都填充满，故将其绝对定位。它的直接子元素水平排列，所以它的显示方式设置为弹性盒子类型：
