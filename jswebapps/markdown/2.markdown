...
基于事件的编程非常强大，因为它能让你的应用架构充分解偶，让功能变得更加内聚且具有更好的可维护性。事件本质上是和DOM无关的，你可以很容易开发出一个事件驱动的库。这种模式称为发布/订阅【译注：更多内容请参照 http://en.wikipedia.org/wiki/Publish/subscribe】，这是很有用的一个模式。
