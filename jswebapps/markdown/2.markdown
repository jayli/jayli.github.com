...
可以从PPK的文章中（http://goo.gl/l7Zqk）查看怪异模式支持的事件类型。
...
和上面提到的函数（stopPropagation()和preventDefault()）一样，event对象还包含很多有用的属性。W3C规范中包含的大部分属性都列在下面，更多信息请参照完整的标准规范(http://goo.gl/PGufh)。
...
基于事件的编程非常强大，因为它能让你的应用架构充分解偶，让功能变得更加内聚且具有更好的可维护性。事件本质上是和DOM无关的，你可以很容易开发出一个事件驱动的库。这种模式称为“发布/订阅”【译注：更多内容请参照 http://goo.gl/lthZa】，这是很有用的一个模式。
